              ...
              <table class='table table-bordered table-striped'>
                ...
                $kel = $this->db->query("SELECT b.kode_kurikulum, c.directory FROM rb_kelas a JOIN rb_tingkat b ON a.id_tingkat=b.id_tingkat JOIN rb_raport c ON b.id_raport=c.id_raport where a.id_kelas='$_GET[kelas]'")->row_array();
                  ...
                  echo "<tr><td>$no</td>
                            ...
                            <td><center><a class='btn btn-success btn-xs' target='_BLANK' title='Penilaian Diri' href='".base_url().$this->uri->segment(1)."/cetak_semester_raport?angkatan=$_GET[angkatan]&kelas=$_GET[kelas]&tahun=$_GET[tahun]&siswa=$r[id_siswa]&page=1'><span class='glyphicon glyphicon-print'></span> 1</a>
                              <a class='btn btn-success btn-xs' target='_BLANK' title='Penilaian Diri' href='".base_url().$this->uri->segment(1)."/cetak_semester_raport?angkatan=$_GET[angkatan]&kelas=$_GET[kelas]&tahun=$_GET[tahun]&siswa=$r[id_siswa]&page=2'><span class='glyphicon glyphicon-print'></span> 2</a>
                              <a class='btn btn-success btn-xs' target='_BLANK' title='Penilaian Diri' href='".base_url().$this->uri->segment(1)."/cetak_semester_raport?angkatan=$_GET[angkatan]&kelas=$_GET[kelas]&tahun=$_GET[tahun]&siswa=$r[id_siswa]&page=3'><span class='glyphicon glyphicon-print'></span> 3</a>
                              <a class='btn btn-success btn-xs' target='_BLANK' title='Penilaian Diri' href='".base_url().$this->uri->segment(1)."/cetak_semester_raport?angkatan=$_GET[angkatan]&kelas=$_GET[kelas]&tahun=$_GET[tahun]&siswa=$r[id_siswa]&page=4'><span class='glyphicon glyphicon-print'></span> 4</a>
                            </center></td>
                        </tr>";
                    $no++;
                    }
