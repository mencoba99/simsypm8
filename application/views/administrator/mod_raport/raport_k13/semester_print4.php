...
$sakit = $this->db->query("SELECT * FROM rb_absensi_siswa_harian a where a.kode_kehadiran='S' AND a.id_siswa='$_GET[siswa]' AND a.id_tahun_akademik='$_GET[tahun]'")->num_rows();
$izin = $this->db->query("SELECT * FROM rb_absensi_siswa_harian a where a.kode_kehadiran='I' AND a.id_siswa='$_GET[siswa]' AND a.id_tahun_akademik='$_GET[tahun]'")->num_rows();
$alpa = $this->db->query("SELECT * FROM rb_absensi_siswa_harian a where a.kode_kehadiran='A' AND a.id_siswa='$_GET[siswa]' AND a.id_tahun_akademik='$_GET[tahun]'")->num_rows();
echo "<b>E. Ketidakhadiran</b>
      <table id='tablemodul1' width=65% border=1>
        <tr><td width='50%'>Sakit</td>  <td> : ".number_format($sakit,0)." Hari</td> </tr>
        <tr><td>Izin</td>               <td> : ".number_format($izin,0)." Hari</td> </tr>
        <tr><td>Tanpa Keterangan</td>   <td> : ".number_format($alpa,0)." Hari</td> </tr>
      </table>";
