<?php
    echo "<div class='col-md-12'>
            ...
            <div class='col-md-12'>";
              ...
              echo form_open_multipart($this->uri->segment(1).'/detail_nilai_observasi_bk/'.$this->uri->segment(3),$attributes); 
              echo "<table class='table table-bordered table-striped'>
                    ...
                    <tbody>";
                    $no = 1;
                    $tampil = $this->model_app->view_join_tigo_where('*','rb_nilai_sikap_butir_bk','rb_siswa','id_siswa','rb_butir_sikap','id_butir_sikap',array('id_kelas'=>$s['id_kelas'],'id_tahun_akademik'=>$thn['id_tahun_akademik'],'status_sikap'=>'spiritual'),'waktu_terjadi','ASC');
                    foreach ($tampil as $r){
                      echo "<tr>
                              <td>$no</td>
                              ...
                              <td><center><a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url().$this->uri->segment(1)."/delete_nilai_observasi_bk/$r[id_nilai_sikap_butir]/".$this->uri->segment(3)."' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></center></td>
                              </tr>";
                        $no++;
                      }
                      echo "</tbody>
            ...
            <div role='tabpanel' class='tab-pane fade' id='produk' aria-labelledby='produk-tab'>
              ...
              echo "<table class='table table-bordered table-striped'>
                    ...
                    <tbody>";
                    $no = 1;
                    $tampil = $this->model_app->view_join_tigo_where('*','rb_nilai_sikap_butir_bk','rb_siswa','id_siswa','rb_butir_sikap','id_butir_sikap',array('id_kelas'=>$s['id_kelas'],'id_tahun_akademik'=>$thn['id_tahun_akademik'],'status_sikap'=>'sosial'),'waktu_terjadi','ASC');
                    foreach ($tampil as $r) {
                      echo "<tr>
                              <td>$no</td>
                              <td>".tgl_view($r['waktu_terjadi'])."</td>
