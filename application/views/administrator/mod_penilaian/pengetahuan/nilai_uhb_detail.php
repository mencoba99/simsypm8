    ...
    echo "<div class='col-md-12'>
              <div class='box box-info'>
                <div class='box-header with-border'>
                  <h3 class='box-title'>Input Nilai UHB</h3>
                </div>
              <div class='box-body'>
            <form action='".base_url().$this->uri->segment(1)."/detail_nilai_uhb/".$this->uri->segment(3)."'' method='GET' class='form-horizontal' role='form'>
            ...
            <div class='panel-body'>
              <ul id='myTabs' class='nav nav-tabs' role='tablist'>
                <li role='presentation' class='active'><a href='#penugasan' id='penugasan-tab' role='tab' data-toggle='tab' aria-controls='penugasan' aria-expanded='true'>Nilai UHB $row[kd]</a></li>
                <li role='presentation' class=''><a href='#rekap' role='tab' id='rekap-tab' data-toggle='tab' aria-controls='rekap' aria-expanded='false'>Rekap Nilai UHB $row[kd]</a></li>
              </ul><br>
            <div id='myTabContent' class='tab-content'>
            <div role='tabpanel' class='tab-pane fade active in' id='penugasan' aria-labelledby='penugasan-tab'>
               <div class='col-md-12'>";
              $attributes = array('class'=>'form-horizontal','role'=>'form');
              echo form_open_multipart($this->uri->segment(1).'/detail_nilai_uhb/'.$this->uri->segment(3),$attributes); 
              ...
              echo "<table class='table table-bordered table-striped'>
                <thead>
                      <tr style='background:#e3e3e3;'>
                        ...
                        <th>NISN</th>
                        <th>Nama Siswa</th>
                        <th><center>Nilai Tes UHB</center></th>
                      </tr>
                    </thead>
                    <tbody>";
                  if ($this->input->get('tanggal')!=''){
                    ...
                    echo "<input type='hidden' name='jumlah' value='".$jumls->num_rows()."'>
                          ...
                          <input type='hidden' name='kategori_nilai' value='12'>
               ...
               <table class='table table-bordered table-striped'>
                <thead>
                  ...
                  if ($this->input->get('tanggal')!=''){
                    ...
                    $jumls = $this->model_app->view_where('rb_siswa',array('id_kelas'=>$s['id_kelas'],'id_identitas_sekolah'=>$this->session->sekolah,'status_siswa'=>'Aktif'));
                    foreach ($siswa as $r) {
                    $aa = $this->db->query("SELECT GROUP_CONCAT(a.nilai SEPARATOR ',') as nilai_uhb, GROUP_CONCAT(DATE_FORMAT(a.tanggal_penilaian,'%d/%m/%Y') SEPARATOR ',') as tanggal_nilai_uhb FROM `rb_nilai_pengetahuan` a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl where a.kategori_nilai='12' AND a.id_siswa='$r[id_siswa]' AND b.id_mata_pelajaran='$s[id_mata_pelajaran]' AND b.id_tahun_akademik='$s[id_tahun_akademik]' AND a.id_kompetensi_dasar='".$this->input->get('kd')."'")->row_array();
                    $nilai_uhb = explode(',',$aa['nilai_uhb']);
                    $rata = array_sum($nilai_uhb)/(count(array_filter($nilai_uhb)));
                    $rataa = $rata;
                    if (is_nan($rataa)) {
                      $rataa = 0;
                    }
                    echo "<tr><td>$no</td>
                              ...
                              <td>$r[nisn]</td>
                              <td>$r[nama]</td>
                              <td><a data-toggle='tooltip' data-placement='right' title='$aa[tanggal_nilai_uhb]' href=''>$aa[nilai_uhb]</a></td>
