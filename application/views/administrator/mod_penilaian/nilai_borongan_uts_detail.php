    ...
    echo "<div class='col-md-12'>
              <div class='box box-info'>
                <div class='box-header with-border'>
                  <h3 class='box-title'>Input Nilai Borongan UTS Siswa</h3>
                  <form style='margin-right:5px; margin-top:0px' class='pull-right' action='".base_url().$this->uri->segment(1)."/import_excel_borongan_uts/import_borongan/".$this->uri->segment(3)."' method='POST' enctype='multipart/form-data'>
                    <a title='Lihat Format File' href='".base_url().$this->uri->segment(1)."/export_nilai_borongan_uts/".$this->uri->segment(3)."'><span class='glyphicon glyphicon-file' aria-hidden='true'></span> Format</a> 
                    ...
                    <a style='margin-top:-4px' class='btn btn-primary btn-sm' href='".base_url().$this->uri->segment(1)."/detail_nilai_borongan_uts/".$this->uri->segment(3)."?ambil=1' onclick=\"return confirm('Yakin ingin ambil Data nilai UTS hingga penginputan nilai saat saat ini?')\"><span class='fa fa-clone' aria-hidden='true'></span> Ambil Penilaian UTS</a>
                  </form>
                </div>
              <div class='box-body'>
            <form action='".base_url().$this->uri->segment(1)."/detail_nilai_borongan_uts/".$this->uri->segment(3)."'' method='GET' class='form-horizontal' role='form'>
            ...
            <div class='col-md-12'>";
            if ($_GET['ambil']=='1'){
              echo "<div class='alert alert-success'><center>Sukses Mengambil Nilai, Klik disini untuk <a href='".base_url().$this->uri->segment(1)."/detail_nilai_borongan_uts/".$this->uri->segment(3)."'>Refresh Halaman</a>.</center></div>";
            }
              $attributes = array('class'=>'form-horizontal','role'=>'form');
              echo form_open_multipart($this->uri->segment(1).'/detail_nilai_borongan_uts/'.$this->uri->segment(3),$attributes); 
              echo "<table class='table table-bordered table-striped'>
                <thead>
                    ...
                    foreach ($siswa as $r) {
                    $ab = $this->db->query("SELECT a.* FROM rb_nilai_borongan_uts a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl where a.id_siswa='$r[id_siswa]' AND b.id_mata_pelajaran='$s[id_mata_pelajaran]' AND a.kodejdwl='".$this->uri->segment(3)."'")->row_array();
                    ...
                    if ($_GET['ambil']=='1'){
                      ...
                      if ($nilai_raport=='0' OR $max_id_kompetensi_dasar==''){
                          $minus = "";
                          $plus = "-";
                      }else{
                      ...
                      foreach ($kompetensi_dasar_ket as $kk) {
                        ...
                        if (max($nilai)==$rata_keterampilan){
                          ...
                          $id_kompetensi_dasar = $kk['id_kompetensi_dasar'];
                      ...
                      if ($nilai_raport_keterampilan!='0' AND $id_kompetensi_dasar!=''){
                        ...
                        $cek = $this->db->query("SELECT * FROM rb_nilai_borongan_uts a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl where a.id_siswa='$r[id_siswa]' AND b.id_mata_pelajaran='$s[id_mata_pelajaran]' AND b.id_tahun_akademik='$s[id_tahun_akademik]'");
                        if ($cek->num_rows() >= '1'){
                    ...
                    echo "<tr><td>$no</td>
                              ...
                              <td>$r[nisn]</td>
                              <td>$r[nama]</td>
                              <input type='hidden' name='id_nilai_borongan_uts".$no."' value='$ab[id_nilai_borongan_uts]'>
