...
if (isset($_POST['login'])){
 ...
 $calon = mysqli_query($koneksi, "SELECT * FROM rb_humas_traceralumni WHERE (username='".anti_injection($_POST['a'])."' OR no_hp='".anti_injection($_POST['a'])."') AND password='$pass'");
 $hitungcalon = mysqli_num_rows($calon);
 if ($hitungcalon >= 1){
    $r = mysqli_fetch_array($calon);
    $_SESSION['id']     = $r['id_traceralumni'];
